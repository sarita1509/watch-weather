...
const getWeatherData = (cityId, tempUnits) => {
  let wd = weatherData.find(ds => ds.id === Number(cityId));
  let convertor = tempUnits === "C" ? k2c : k2f;
...
export function weatherServer({ environment = "development" } = {}) {
  return new Server({
    environment,
    routes() {
      ...
      this.get("/current/:cityId/:tempUnits", (_, request) => {
        const city =
          Number(request.params.cityId) !== 0 ? request.params.cityId : 1277333;
